...
def _make_sbom(repo="lpasquali/rune", components=None):
    return SBOMDocument(bomFormat="CycloneDX", specVersion="1.5",
        ...
        tools=[SBOMToolMetadata(vendor="Anchore", name="syft", version="1.4.0")], source_repo=repo)

def _make_cve_scan(repo="lpasquali/rune", findings=None):
    return CVEScanResult(
        findings=findings or [
            CVEFinding(cve_id="CVE-2024-1234", severity=CVESeverity.HIGH, cvss_score=8.8,
                       package_name="test-pkg", package_version="1.0.0", fixed_version="2.0.1"),
            CVEFinding(cve_id="CVE-2005-2541", severity=CVESeverity.LOW, cvss_score=2.1,
                       package_name="tar", package_version="1.35"),
        ], scanner_name="grype", source_repo=repo)

def _make_vex_doc(repo="lpasquali/rune"):
    return VEXDocument(**{"@context": "https://openvex.dev/ns/v0.2.0", "@id": "urn:test:vex:1"},
        author="test", timestamp="2026-04-01T00:00:00+00:00", version=1,
...
def _make_slsa(repo="lpasquali/rune", verified=True):
    return SLSAAttestation(subject_name="ghcr.io/lpasquali/rune",
        builder_id="https://github.com/actions/runner/",
        build_type="https://actions.github.io/buildtypes/workflow/v1",
        source_repo=repo, verified=verified)

def _make_full_bundle():
    return EvidenceBundle(repos=["lpasquali/rune"], sboms=[_make_sbom()],
        cve_scans=[_make_cve_scan()], vex_documents=[_make_vex_doc()],
