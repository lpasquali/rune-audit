...
class TestVEXValidatorDocument:
    ...
    def test_not_affected_without_justification(self) -> None:
        doc = _make_vex_doc(
            [{"vulnerability": {"name": "CVE-X"}, "status": "not_affected", "impact_statement": "Impact."}]
        )
        result = VEXValidator().validate_document(doc)
    ...
    def test_justification_strength_cnp(self) -> None:
        doc = _make_vex_doc(
            [{"vulnerability": {"name": "CVE-X"}, "status": "not_affected", "justification": "component_not_present"}]
        )
        assert VEXValidator().validate_document(doc).has_warnings
    ...
    def test_result_properties(self) -> None:
        doc = _make_vex_doc(
            [
                {"vulnerability": {"name": "CVE-1"}, "status": "not_affected", "impact_statement": "OK"},
                {"vulnerability": {"name": "CVE-2"}, "status": "affected", "impact_statement": "V."},
            ]
        )
        result = VEXValidator().validate_document(doc)
...
class TestVEXValidatorCrossCheck:
            ...
            [
                {
                    ...
                    "status": "not_affected",
                    "justification": "component_not_present",
                    "impact_statement": "Old.",
                }
            ]
        )
        scan = CVEScanResult(
            findings=[CVEFinding(cve_id="CVE-X", severity=CVESeverity.HIGH, package_name="pkg", fixed_version="2.0.0")],
            scanner_name="grype",
        )
    ...
    def test_no_issues(self) -> None:
        ...
        assert not any(
            f.severity == ValidationSeverity.WARNING for f in VEXValidator().cross_check([doc], [scan]).findings
        )
