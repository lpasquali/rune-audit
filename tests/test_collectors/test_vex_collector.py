...
class TestVEXCollector:
    ...
    def test_fetch_vex(self) -> None:
        ...
        collector = self._make_collector(
            {"/repos/lpasquali/rune/contents/": (200, _make_vex_content_response(vex_data))}
        )
    ...
    def test_fetch_vex_invalid_encoding(self) -> None:
        collector = self._make_collector(
            {"/repos/lpasquali/rune/contents/": (200, {"content": "test", "encoding": "utf-8"})}
        )
        assert collector.fetch_vex("lpasquali/rune") is None
        collector.close()
    ...
    def test_fetch_vex_invalid_json(self) -> None:
        ...
        collector = self._make_collector(
            {"/repos/lpasquali/rune/contents/": (200, {"content": content, "encoding": "base64"})}
        )
        assert collector.fetch_vex("lpasquali/rune") is None
        collector.close()
    ...
    def test_collect_all(self) -> None:
        ...
        collector = self._make_collector(
            {
                "/repos/lpasquali/rune/contents/": (200, _make_vex_content_response(vex_data)),
                "/repos/lpasquali/rune-docs/contents/": (200, _make_vex_content_response(vex_data)),
            }
        )
