...
def test_from_openvex():
    data = {"vulnerability": {"name": "CVE-X"}, "products": [{"@id": "pkg:a"}], "status": "not_affected",
            "justification": "vulnerable_code_cannot_be_controlled_by_adversary", "impact_statement": "OK"}
...
def test_from_openvex_affected():
    s = VEXStatement.from_openvex({"vulnerability": {"name": "X"}, "status": "affected", "impact_statement": "V", "action_statement": "F"})
    assert s.status == VEXStatus.AFFECTED and s.action_statement == "F"

def test_from_openvex_string_vuln():
    assert VEXStatement.from_openvex({"vulnerability": "CVE-X", "status": "under_investigation"}).vulnerability_name == "CVE-X"

def test_from_openvex_invalid_just():
    assert VEXStatement.from_openvex({"vulnerability": {"name": "X"}, "status": "not_affected", "justification": "bad"}).justification is None
...
def test_from_openvex_invalid_product():
    assert VEXStatement.from_openvex({"vulnerability": {"name": "X"}, "status": "fixed", "products": [{"bad": "x"}]}).products == []
