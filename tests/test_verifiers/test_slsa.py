...
class TestChecks:
    ...
    def test_src_sha256(self) -> None:
        b = AttestationBundle(
            repo="t", tag="v", found=True, payload={"predicate": {"materials": [{"digest": {"sha256": "aa"}}]}}
        )
    ...
    def test_iso_fail(self) -> None:
        b = AttestationBundle(repo="t", tag="v", found=True, payload={"predicate": {"buildType": "c", "metadata": {}}})
...
class TestReport:
    ...
    def test_gap(self) -> None:
        ...
        r.checks.append(
            SLSACheckResult(
                requirement=SLSARequirement.BUILD_PROVENANCE_EXISTS, status=VerificationStatus.FAIL, message="m"
            )
        )
